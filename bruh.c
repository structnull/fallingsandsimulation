...
float velocityGrid[rows][cols];
float nextvelocityGrid[rows][cols];
...
void initializeGrid()
{
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            grid[i][j] = 0;
            nextgrid[i][j] = 0;
            velocityGrid[i][j] = 0.0f;
            nextvelocityGrid[i][j] = 0.0f;
        }
    }
}
...
void drawGrid()
{
    for (int i = rows - 1; i >= 0; i--)
    {
        for (int j = 0; j < cols; j++)
        {
            ...
            if (grid[i][j] == 1)
            {
                ...
                int newPos = i + (int)velocity;
                for (int y = newPos; y > i; y--)
                {
                    if (i + 1 < rows)
                    {
                        if (j - 1 >= 0)
                    ...
                    if (i + 1 < rows && below == 0 && belowA == 0 && belowB == 0)
                    {
                        nextgrid[i][j] = 0;
                        nextgrid[i + 1][j] = 1;
                        nextvelocityGrid[y][j] = velocity + gravity;
                        moved = true;
                    }
                }
    ...
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
            ...
            velocityGrid[i][j] = nextvelocityGrid[i][j];
            nextgrid[i][j] = 0;
        }
    }
...
void handleMouseInput()
{
    if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
    {
        ...
        int gridX = (int)(mousePosition.x / cellSize);
        int gridY = (int)(mousePosition.y / cellSize);
